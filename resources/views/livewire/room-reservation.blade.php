<div>
    <form wire:submit.prevent="reserve" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 dark:bg-gray-800">
            ...
            <div class="flex space-x-4">
                <div wire:ignore class="w-1/2">
                    <input x-data x-init="flatpickr($el, {
                        ...
                        placeholder="{{ __('messages.check_in') }}" value="{{ $check_in }}">
                </div>
                <div wire:ignore class="w-1/2">
                    <input x-data x-init="flatpickr($el, {
                        enableTime: false,
                        ...
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 dark:bg-gray-700"
                        placeholder="{{ __('messages.check_out') }}" value="{{ $check_out }}">
                </div>
            </div>
            @error('check_in')
                <span class="text-red-500">{{ $message }}</span>
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-4 border rounded dark:border-gray-600">
                    ...
                    <div class="flex items-center space-x-4">
                        <button type="button" wire:click="removeGuest('adults')"
                        ...
                        <button type="button" wire:click="addGuest('adults')"
                ...
                <div class="flex items-center justify-between p-4 border rounded dark:border-gray-600">
                    ...
                    <div class="flex items-center space-x-4">
                        <button type="button" wire:click="removeGuest('teenagers')"
                            class="bg-red-500 hover:bg-red-600 text-white w-8 h-8 rounded-full">
                            -
                        </button>
                        <span class="w-8 text-center dark:text-white">{{ $guests['teenagers'] }}</span>
                        <button type="button" wire:click="addGuest('teenagers')"
                            class="bg-green-500 hover:bg-green-600 text-white w-8 h-8 rounded-full">
                            +
                        </button>
                    </div>
                ...
                <div class="flex items-center justify-between p-4 border rounded dark:border-gray-600">
                    ...
                    <div class="flex items-center space-x-4">
                        <button type="button" wire:click="removeGuest('children')"
                            class="bg-red-500 hover:bg-red-600 text-white w-8 h-8 rounded-full">
                            -
                        </button>
                        <span class="w-8 text-center dark:text-white">{{ $guests['children'] }}</span>
                        <button type="button" wire:click="addGuest('children')"
                            class="bg-green-500 hover:bg-green-600 text-white w-8 h-8 rounded-full">
                            +
                        </button>
                    </div>
                ...
                <div class="flex items-center justify-between p-4 border rounded dark:border-gray-600">
                    ...
                    <div class="flex items-center space-x-4">
                        <button type="button" wire:click="removeGuest('infants')"
                            class="bg-red-500 hover:bg-red-600 text-white w-8 h-8 rounded-full">
                            -
                        </button>
                        <span class="w-8 text-center dark:text-white">{{ $guests['infants'] }}</span>
                        <button type="button" wire:click="addGuest('infants')"
                            class="bg-green-500 hover:bg-green-600 text-white w-8 h-8 rounded-full">
                            +
                        </button>
                    </div>
                ...
                <div class="flex items-center justify-between p-4 border rounded dark:border-gray-600">
                    ...
                    <div class="flex items-center space-x-4">
                        <button type="button" wire:click="removeGuest('seniors')"
                            class="bg-red-500 hover:bg-red-600 text-white w-8 h-8 rounded-full">
                            -
                        </button>
                        <span class="w-8 text-center dark:text-white">{{ $guests['seniors'] }}</span>
                        <button type="button" wire:click="addGuest('seniors')"
                            class="bg-green-500 hover:bg-green-600 text-white w-8 h-8 rounded-full">
                            +
                        </button>
                    </div>
        ...
        @endif
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2 dark:text-white" for="voucher_code">
                {{ __('messages.voucher_code') }}
            </label>
            <div class="flex space-x-2">
                <input wire:model="voucher_code"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:text-gray-300 dark:bg-gray-700"
                    id="voucher_code" type="text" placeholder="{{ __('messages.enter_voucher_code') }}">
            ...
            @if (session()->has('voucher_success'))
                <span class="text-green-500 text-sm">{{ session('voucher_success') }}</span>
            @endif
            @if (session()->has('voucher_error'))
                <span class="text-red-500 text-sm">{{ session('voucher_error') }}</span>
            @endif
        </div>
        <div class="mb-6 space-y-2">
            @if ($discount_amount > 0)
                <p class="text-base text-gray-600 dark:text-gray-400">
                    {{ __('messages.original_price') }}: <span
                        class="line-through">{{ number_format($original_price, 2) }} PLN</span>
                </p>
                <p class="text-base text-green-600">
                    {{ __('messages.discount') }}: -{{ number_format($discount_amount, 2) }} PLN
                </p>
            @endif
            <p class="text-lg font-bold">
                {{ __('messages.total_price') }}: {{ number_format($total_price, 2) }} PLN
            </p>
        </div>
